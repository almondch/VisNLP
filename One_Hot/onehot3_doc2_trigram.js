d3.json("One_Hot/ohe_step2_doc2_trigram.json", function(data) {
	...
	var svg = d3.select("#ohe_step3").append("svg")
	...
	var tooltip = d3.select("#ohe_step3")
		.append("div")
		.attr('class', 'tooltip');
	...
	var node = svg.selectAll(".node")
		...
		.on("mouseover", function(d) {
			if (d.name != "job2") return tooltip.style("visibility", "visible").text(d.name + ' : ' + d.value1);})
	...
	node.append("circle")
		.attr("r", 6)
		.attr("fill", function(d) { return d.children || d._children ? "white": myColor(d.value1);})
